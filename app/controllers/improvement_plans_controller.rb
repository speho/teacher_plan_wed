class ImprovementPlansController < ApplicationController
  before_action :current_user_must_be_improvement_plan_coach, :only => [:edit, :update, :destroy]

  def current_user_must_be_improvement_plan_coach
    improvement_plan = ImprovementPlan.find(params[:id])

    unless current_user == improvement_plan.coach
  ...
  def index
    ...
    @improvement_plans = @q.result(:distinct => true).includes(:comments, :coach, :collaborators).page(params[:page]).per(10)
  ...
  def create
    ...
    @improvement_plan.summary = params[:summary]
